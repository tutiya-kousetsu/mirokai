...
void PostEffect::Initialize()
{
	...
	srvDescHeapDesc.NumDescriptors = 2;//テクスチャ枚数
	...
	for (int i = 0; i < 2; i++) {
		//デスクリプタヒープにSRV作成
		device->CreateShaderResourceView(texBuff[i].Get(),//ビューと関連付ける
			&srvDesc,//テクスチャ設定情報
			CD3DX12_CPU_DESCRIPTOR_HANDLE(
				descHeapSRV->GetCPUDescriptorHandleForHeapStart(), i,//ヒープの先頭アドレス)
