...
void Fbx_Object3d::Update()
{
	...
	//モデルのメッシュトランスフォーム
	const XMMATRIX& modelTransform = fbx_model->GetModelTransform();
	//カメラ座標
	...
	//ボーン配列
	std::vector<Fbx_Model::Bone>& bones = fbx_model->GetBones();
	//アニメーション
	if (isPlay) {
		//1フレーム進める
...
{
	FbxScene* fbxScene = fbx_model->GetFbxScene();
	//0番のアニメーション
